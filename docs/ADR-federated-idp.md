...
CREATE TABLE IF NOT EXISTS identity_providers (
    ...
    issuer TEXT NOT NULL,                  -- JWT issuer URL (globally unique)
...
func (r *robotjwt) Generate(req *http.Request) security.Context {
    ...
    idp, _ := federatedidp.Ctl.GetIdpByIssuer(req.Context(), issuer)
...
| 401 | Unauthorized |
| 403 | Forbidden |
| 409 | Conflict (name or issuer already exists) |
...
| **Issuer** | HTTPS URL only, **globally unique** (system + all projects), immutable |
| **Claim Path** | Max 128 characters |
| **Claim Value** | Max 256 characters |

**Issuer Uniqueness:**
- Cannot create project-level IdP with same issuer as system-level IdP
- Cannot create system-level IdP with same issuer as any project-level IdP
- Validated at creation time, not updateable
...
# No IdP found
"no IdP found for issuer https://example.com"
