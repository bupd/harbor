...
3. Check `name` uniqueness within project
...
9. Check `issuer` global uniqueness
...
5. Check `issuer` global uniqueness
...
- Issuer uniqueness check should be atomic
- Consider race conditions in CREATE
...
| `identity_provider_id` | Required, must exist |
...
- [x] Issuer uniqueness (global)
- [x] Name uniqueness (per project)
